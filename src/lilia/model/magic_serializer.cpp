#include "lilia/model/magic_serializer.hpp"

#include <fstream>
#include <iomanip>
#include <sstream>

#include "lilia/model/core/magic.hpp"

namespace lilia::model::magic {

namespace {

std::string bitboard_to_hex(bb::Bitboard v) {
  std::ostringstream oss;
  oss << "0x" << std::hex << std::setw(16) << std::setfill('0') << v;
  return oss.str();
}

}  // namespace

bool serialize_magics_to_header(const std::string& outPath, const std::string& prefix) {
  const auto& rmag = rook_magics();
  const auto& bmag = bishop_magics();
  const auto& rtab = rook_tables();
  const auto& btab = bishop_tables();

  std::ofstream out(outPath, std::ios::out | std::ios::trunc);
  if (!out.is_open()) return false;

  out << "#pragma once\n\n";
  out << "#include <array>\n#include <vector>\n#include <cstdint>\n#include \"../core/model_types.hpp\"\n\n";
  out << "// Generated magic constants & attack tables\n";
  out << "// Generated by magic_serializer - include this header to skip runtime magic search\n\n";
  out << "#define LILIA_MAGIC_HAVE_CONSTANTS 1\n\n";
  out << "namespace lilia::model::magic::constants {\n\n";

  out << "struct MagicEntry { bb::Bitboard magic; std::uint8_t shift; };\n\n";

  // Rook magics
  out << "static inline const std::array<MagicEntry, 64> " << prefix << "_rook_magic = {\n    {";
  for (size_t i = 0; i < 64; ++i) {
    out << "{ " << bitboard_to_hex(rmag[i].magic) << ", " << static_cast<unsigned>(rmag[i].shift)
        << " }";
    if (i + 1 < 64) out << ", ";
    if ((i + 1) % 4 == 0) out << "\n    ";
  }
  out << "\n    }\n};\n\n";

  // Bishop magics
  out << "static inline const std::array<MagicEntry, 64> " << prefix << "_bishop_magic = {\n    {";
  for (size_t i = 0; i < 64; ++i) {
    out << "{ " << bitboard_to_hex(bmag[i].magic) << ", " << static_cast<unsigned>(bmag[i].shift)
        << " }";
    if (i + 1 < 64) out << ", ";
    if ((i + 1) % 4 == 0) out << "\n    ";
  }
  out << "\n    }\n};\n\n";

  // Rook attack tables
  out << "// rook attack tables\n";
  out << "static inline const std::array<std::vector<bb::Bitboard>, 64> " << prefix
      << "_rook_table = {\n    {\n";
  for (size_t i = 0; i < 64; ++i) {
    out << "        std::vector<bb::Bitboard>{";
    const auto& vec = rtab[i];
    for (size_t j = 0; j < vec.size(); ++j) {
      out << bitboard_to_hex(vec[j]);
      if (j + 1 < vec.size()) out << ", ";
    }
    out << "}";
    if (i + 1 < 64)
      out << ",\n";
    else
      out << "\n";
  }
  out << "    }\n};\n\n";

  // Bishop attack tables
  out << "// bishop attack tables\n";
  out << "static inline const std::array<std::vector<bb::Bitboard>, 64> " << prefix
      << "_bishop_table = {\n    {\n";
  for (size_t i = 0; i < 64; ++i) {
    out << "        std::vector<bb::Bitboard>{";
    const auto& vec = btab[i];
    for (size_t j = 0; j < vec.size(); ++j) {
      out << bitboard_to_hex(vec[j]);
      if (j + 1 < vec.size()) out << ", ";
    }
    out << "}";
    if (i + 1 < 64)
      out << ",\n";
    else
      out << "\n";
  }
  out << "    }\n};\n\n";

  out << "} // namespace lilia::model::magic::constants\n";
  out.close();
  return true;
}

}  // namespace lilia::model::magic
