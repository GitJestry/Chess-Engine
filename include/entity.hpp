...
public:
  ...
  // This means whenether the position of this entity or scale will be changed, it always
  ...
  // the width and height fraction are expressing how the sprite should be scaled
  // for example, width_fraction = 0.5 means 50% of the original width
  void setScale(float width_fraction, float height_fraction);

private:
  // not protected because every entity class should only operate and make changes to the sprite via member-functions
  sf::Sprite sprite_;
};
